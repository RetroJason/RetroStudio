...
function fileReasonName(reasonCode) {
  return FILE_ERROR_REASON[reasonCode] || `UNKNOWN_REASON(${reasonCode})`;
}
...
class RetroWatchBleClient {
  ...
  async fileBegin(totalSize, imageCrc32, opts = {}) {
    ...
    const packet = await this.sendRequest(MSG.FILE_BEGIN_REQ, payload, {
      expectType: MSG.FILE_BEGIN_RSP,
      timeoutMs: 6000,
      requestId: reqId,
    });
    ...
    this._emitDebug({
      ...
      reason,
      reasonName: fileReasonName(reason),
  ...
  async fileChunk(offset, chunkBytes) {
    const reqId = this._nextRequestId();
    const payload = concatBytes([
      u16LE(reqId),
      u32LE(offset >>> 0),
    ...
    const packet = await this.sendRequest(MSG.FILE_CHUNK_REQ, payload, {
      expectType: MSG.FILE_CHUNK_RSP,
      timeoutMs: 6000,
      requestId: reqId,
    });
  ...
  async fileCommit() {
    ...
    const packet = await this.sendRequest(MSG.FILE_COMMIT_REQ, payload, {
      expectType: MSG.FILE_COMMIT_RSP,
      timeoutMs: 10000,
      requestId: reqId,
    });
  ...
  async launchApp(path, args = "") {
    const reqId = this._nextRequestId();
    const pathBytes = textToBytes(String(path));
    const argsBytes = textToBytes(String(args || ""));
    const payload = concatBytes([
      u16LE(reqId),
      u16LE(pathBytes.length),
      pathBytes,
      u16LE(argsBytes.length),
      argsBytes,
    ]);

    const packet = await this.sendRequest(MSG.LAUNCH_APP_REQ, payload, {
      expectType: MSG.LAUNCH_APP_RSP,
      timeoutMs: 6000,
      requestId: reqId,
    });
  ...
  async sendFile(data, opts = {}) {
    ...
    if (beginRsp.status !== STATUS.OK) {
      ...
      return {
        ...
        error: {
          ...
          reason: beginReason,
          reasonName: fileReasonName(beginReason),
    ...
    const enqueueChunk = () => {
      ...
      this._emitDebug({
        phase: "file_chunk_tx",
        msgType: MSG.FILE_CHUNK_REQ,
        msgName: msgTypeName(MSG.FILE_CHUNK_REQ),
        offset: chunkOffset,
        length: chunk.length,
      });
      ...
      inFlight.push({
        offset: chunkOffset,
        length: chunk.length,
        promise: this.fileChunk(chunkOffset, chunk),
      });
    ...
    const commitRsp = await this.fileCommit();
    if (commitRsp.status !== STATUS.OK) {
      ...
      return {
        ...
        error: {
          ...
          reason: commitReason,
          reasonName: fileReasonName(commitReason),
